...
Prepared 2026-09-14 and corrective-review updated 2026-09-15. The reviewed schematic is under `hardware/pcb/kicad/`.
...
1. **JMS583 exposed pad:** pin 65 `EP` is explicitly connected to GND, matching the grounded center pad used by the available JMS583 package/reference guidance. This connection must be implemented with an appropriate thermal/via structure during layout.
...
3. **JMS583 application circuits:** the corrective review now uses 1.0 V for every VCCK/AVDDL pin, a 4.7 uH LXO inductor, a 330 kΩ/1 uF reset RC, 30 pF crystal loads plus 510 kΩ XIN-XOUT bias, grounded TME/EPAD, a GPIO4 active-low LED, and a GPIO6 VBUS divider. Crystal parasitics and final passive selections remain release checks.
...
- **Pinout/package uncertainties:** TPD4E05U06 DQA0010A versus DQA0010B construction remains; JMS583 EPAD is now explicitly grounded and its thermal/via implementation remains a layout requirement.
...
- **Schematic validation:** after the 2026-09-15 corrective review, KiCad 9.0.5 parses and exports netlist/BOM/PDF/SVG and reports **0 ERC errors and 0 ERC warnings**.

## Corrected schematic summary

The single A3 schematic is divided into four labeled sections: USB interface, JMS583 bridge, NVMe/M.2, and power. It contains the exact USB-C receptacle, JMS583, M.2 socket, TPS54302, crystal, flash and three TPD4E05U06 arrays. Four 100 nF capacitors couple the JMS583 USB transmitter pairs, and four 220 nF capacitors couple the JMS583 PCIe transmit lanes. Only PCIe lanes 0 and 1 are connected; lanes 2 and 3 are explicitly unconnected. The ESD arrays are shunt branches from continuous connector-side USB nets and are not series elements.

The former U5 power-mux placeholder has been removed. `USB5V` powers the JMS583 VDDREG/VBUS side and its internally generated 3.3 V/1.0 V rails; `AUX5V` independently powers the TPS54302 and NVMe 3.3 V rail; grounds are common. AUX5V/3V3_NVME must be ready before enumeration when SSD access is required. The TPS54302 network retains TI's 3.3 V starting values: 6.8 uH, 44 uF nominal output capacitance, 100 kΩ / 22.1 kΩ feedback and 47 pF feed-forward. The regulator's 3 A maximum is a release gate until the 4 TB SSD is selected.
